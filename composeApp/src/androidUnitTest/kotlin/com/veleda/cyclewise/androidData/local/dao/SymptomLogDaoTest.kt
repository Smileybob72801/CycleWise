...
class SymptomLogDaoTest : KoinTest {
    ...
    // --- Test Data ---
    private val parentEntry1 = DailyEntryEntity("entry-1", LocalDate(2025, 1, 5), 5, customTags = "[]", note = null, createdAt = TestData.INSTANT, updatedAt = TestData.INSTANT)
    private val parentEntry2 = DailyEntryEntity("entry-2", LocalDate(2025, 1, 6), 6, customTags = "[]", note = null, createdAt = TestData.INSTANT, updatedAt = TestData.INSTANT)
    private val parentSymptom1 = SymptomEntity("symptom-1", "Cramps", SymptomCategory.PAIN, TestData.INSTANT)
    private val parentSymptom2 = SymptomEntity("symptom-2", "Anxiety", SymptomCategory.MOOD, TestData.INSTANT)

    private val log1_symptom1_entry1 = SymptomLogEntity("log-1", "entry-1", "symptom-1", 4, TestData.INSTANT)
    private val log2_symptom2_entry1 = SymptomLogEntity("log-2", "entry-1", "symptom-2", 2, TestData.INSTANT)
    private val log3_symptom1_entry2 = SymptomLogEntity("log-3", "entry-2", "symptom-1", 5, TestData.INSTANT)
    ...
    fun insertAll_WHEN_logWithSameIdExists_THEN_onConflictReplaceUpdatesTheRecord() = runTest {
        ...
        dailyEntryDao.insert(parentEntry1)
        symptomDao.insert(parentSymptom1)
        dao.insertAll(listOf(log1_symptom1_entry1))
